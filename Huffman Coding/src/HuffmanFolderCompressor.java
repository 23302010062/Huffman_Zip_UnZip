...
public class HuffmanFolderCompressor {
    ...
    public void compressFolder() throws IOException {
        try (FileOutputStream fos = new FileOutputStream(outputFile);
            ...
            // 遍历文件夹
            traverseAndCompress(sourceFolder, "", compressedData, codingSets);
    ...
    private void traverseAndCompress(File file, String relativePath,
                                     ...
                                     Map<String, Map<Byte, String>> codingSets) throws IOException {
        if (file.isDirectory()) {
            // 如果是文件夹，递归遍历子文件
            File[] children = file.listFiles();
            if (children.length != 0) {
                for (File child : children) {
                    traverseAndCompress(child, relativePath + "/" + child.getName(), compressedData, codingSets);
                }
            } else {
